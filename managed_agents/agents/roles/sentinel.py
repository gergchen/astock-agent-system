...
# 冷却时间（秒）
HOTSPOT_COOLDOWN = 300    # 热点 TOP3 变化：5分钟内不重复
GLOBAL_COOLDOWN = 120     # 全局告警：2分钟内最多发一批
...
class Sentinel(BaseAgent):
    ...
    def _is_trading_time() -> bool:
        """判断当前是否在A股交易时段（9:25-15:05，周一至周五）。"""
        now = datetime.now()
        if now.weekday() >= 5:
            return False
